...
class PicoBillBoard:
    ...
    def __init__(self):
        ...
        # do the handling
        self._nextB = Button(10, "button1", buttonhandler=None)
        self._previousB = Button(12, "button3", buttonhandler=None)
        self._partyB = Button(11, "button2", buttonhandler=None)
        ...
        self._adList = {} # Jsons are Dictionaries
        self._index = -1
        ...
        self._model.addButton(self._nextB)
        self._model.addButton(self._partyB)
        self._model.addButton(self._previousB)
        ...
        self._model.addTransition(3, [TIMEOUT], 3)
        self._model.addTransition(3, [BTN2_PRESS], 4)
        self._model.addTransition(4, [BTN2_PRESS], 3)
        # etc.
    ...
    def pullServerList(self):
        try:
            ads = open('sample_ad_list.json', 'r') # wont find this file
            self._adList = json.load(ads)
            ads.close()
        except OSError: # Error catch
            ads = open('sample_ad_list1.json', 'r') # will find this file
            self._adList = json.load(ads)
            ads.close()
            self._model.gotoState(3)
        except: # Go to state 2 to let the user know there was an error, and try again
    ...
    def stateDo(self, state):
        ...
        if state == 0:
            ...
            self._timer.check()
            self._display.showText(str(self._adList["Ad_List"][self._index]["Ad_Text"]))
            if self._nextB.isPressed(): 
                self._model.gotoState(3)
            elif self._previousB.isPressed(): 
    ...
    def stateEntered(self, state, event):
        ...
        if state == 0:
            ...
            self._index=-1
            self._display.showText(f"                               ")
            self._display.showText(f"YOUR AD HERE    PICO ADS ")
        elif state == 1:
            # entry actions for state 1
            print('State 1 entered')
            # Load the ads
            self.pullServerList()
            print(self._adList) # Print adList to test
        elif state == 2:
            # Entry actions for state 2
            print('State 2 entered')
            self._display.showText(f"                               ")
            self._display.showText(f"Error pulling adlist           ")
            self._timer.start(5)
        elif state == 3:
            # Entry actions for state 2
            ...
            self._display.reset()
            self._index +=1
            if self._index > len(self._adList):
                self._model.gotoState(0)
            self._timer.start(self._adList["Ad_List"][self._index]["Time_s"])
        elif state == 4:
            # Entry actions for state 2
            print('State 4 entered')
            self._partylight.run(NeoPixel.RAINBOW)
